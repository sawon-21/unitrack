...
export const PostCard: React.FC<PostCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'border-b border-slate-800 p-4 transition-colors duration-200 hover:bg-slate-900/40 flex flex-col gap-2',
        'bg-slate-950/20'
      )}
    >
      {post.repostedBy && post.repostedBy.length > 0 && (
        ...
        >
          ...
          {post.repostedBy.length === 1
            ? `@${post.repostedBy[0]}`
            : post.repostedBy.length === 2
            ? `@${post.repostedBy[0]} and @${post.repostedBy[1]}`
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 text-base truncate">
            <span className="font-bold text-slate-400 truncate hover:text-slate-300 transition-colors">
              @{authorHandle}
            </span>
          ...
          <h2 className="font-bold text-slate-100 mt-0.5 text-lg leading-snug">{post.title}</h2>
              ...
              components={{
                a: ({ node, children, href, ...props }) => {
                  const getPostId = (url: string) => {
                    try {
                      if (url.startsWith('?post=')) return new URLSearchParams(url).get('post');
                  ...
                  if (Array.isArray(children) && children.length === 1 && typeof children[0] === 'string' && children[0].startsWith('http')) {
                    try { linkText = `${new URL(children[0]).origin}/...`; } catch {}
                  } else if (typeof children === 'string' && children.startsWith('http')) {
                    try { linkText = `${new URL(children).origin}/...`; } catch {}
                  }
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-0.5 mt-1">
              ...
              {post.tags.length > 3 && (
                <span className="text-[10px] text-slate-500 px-1 py-0.5">+{post.tags.length - 3} more</span>
              )}
            </div>
          )}
            ...
            <div className={cn('mt-3 grid gap-2', images.length > 1 ? 'grid-cols-2' : 'grid-cols-1')}>
              {images.slice(0, 2).map((url, index) => (
                <div
                  key={`${url}-${index}`}
                  className={cn(
                    'rounded-xl overflow-hidden border border-slate-800 bg-slate-950 cursor-zoom-in relative',
                    images.length > 1 ? 'aspect-[4/5] sm:aspect-square' : 'max-h-[600px]'
                  )}
                  onClick={(e) => {
                    e.stopPropagation();
          ...
          <ImageModal imageUrls={isZoomed ? images : null} initialIndex={zoomedImageIndex} onClose={() => setIsZoomed(false)} />

          <div className="flex items-center gap-2 mt-3">
            {showStatus && (
              <span
                className={cn(
                  'px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border cursor-pointer hover:opacity-80 transition-opacity',
          ...
          <div className="flex items-center justify-between mt-3 text-slate-500 max-w-md">
            ...
            >
              <div className="p-2 -m-2 rounded-full group-hover:bg-emerald-500/10">
                <ThumbsUp className={cn('w-5 h-5', isLiked && 'fill-emerald-500')} />
              </div>
            ...
            >
              <div className="p-2 -m-2 rounded-full group-hover:bg-pink-500/10">
                <ThumbsDown className={cn('w-5 h-5', isDisliked && 'fill-pink-500')} />
              </div>
              <span className="text-sm">{post.dislikes || 0}</span>
            </button>
            {/* Views — wired to navigate to post detail */}
            <button
              onClick={(e) => { e.stopPropagation(); onClick(); }}
              className="flex items-center gap-2 hover:text-sky-400 group transition-colors"
            >
